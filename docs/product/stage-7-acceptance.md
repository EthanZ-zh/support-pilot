...
- 后端：66 passed，语句覆盖率 93%，Ruff 与 strict mypy 通过；
...
- 容器：两次 build 都在读取 Dockerfile 后、执行项目构建前，因 Docker Hub token 地址 TCP 超时失败。

## 剩余风险

- API/前端镜像没有在本机完成 build 和启动，待网络恢复后执行 `docker compose build api frontend && docker compose up -d`；
- CI 文件尚未在真实 GitHub 仓库 runner 上执行，本目录当前不是 Git 仓库；
