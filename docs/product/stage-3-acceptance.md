...
- [x] 50 个检索正例及 10 个 Answerability 负例的 calibration 评测，及 20 正例 + 10 负例的独立 holdout。
...
- local_bge calibration：Recall@5/MRR/nDCG@5/Answerability F1 均为 1.000；
- local_bge holdout：Recall@5 1.000、MRR 0.975、nDCG@5 0.982、Answerability F1 0.974，FP=0、FN=1；
- 本地模型：`BAAI/bge-small-zh-v1.5` 与 `BAAI/bge-reranker-base` 从 D 盘缓存加载成功；
- 数据性质：上述指标来自人工标注合成样本，不是生产准确率。
...
- Answerability：holdout F1 达到首版目标；实现冻结后首次评测保留 1 条 local BGE 安全升级，未反向调整阈值。
- 引用：验证了标注来源召回与引用字段映射，尚未评估生成答案逐主张引用正确率和忠实度。

## 5. 剩余风险

- PostgreSQL `simple` 分词对中文正文较弱；当前技术标识和精确短语较多，不能外推到大型中文知识库。
- calibration 与 holdout 已分离，但两者仍来自同一个合成业务域，样本量也小；指标不能外推到大型或真实企业知识库。
